...
const removeDomain = (domain: string) => {
  console.log("Remover domínio:", domain);
};
...
<template>
  <Container tag="main" class="space-y-8" v-if="project">
    <PageTitle class="mb-5 md:mb-6 lg:mb-8">
      <template #title> Gerenciar Projeto </template>
      <template #subtitle>
        Gerencie as configurações, chaves de API e domínios do projeto
    ...
    <section class="space-y-5">
        ...
        <template #content>
          <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            <InputGroup inputId="name">
              <template #label> Nome do Projeto </template>
              <template #input>
      ...
      <Card>
        ...
        <template #content>
          <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div class="flex items-center gap-3" v-if="project.active">
            ...
            <div class="flex items-center gap-3" v-else>
              ...
              <p class="text-sm text-warning bg-warning/10 px-3 py-1 rounded-full">
                Inativo
              </p>
            </div>
          ...
          <div class="space-y-5">
            <div class="space-y-3">
              <p class="text-sm text-zinc-600 dark:text-zinc-400">
                Adicione os domínios que podem ser usados para enviar e-mails através
                deste projeto.
              </p>
              <InputGroup inputId="domain" class="flex-1">
                <template #label> Novo Domínio </template>
                <template #input>
            ...
            <div class="space-y-3">
              ...
              <ul class="space-y-2 text-zinc-800 dark:text-zinc-200 text-sm">
                <li v-for="domain in project.allowedOrigins" :key="domain"
      ...
      <Card>
        ...
        <template #content>
          <div class="space-y-3">
            <p class="text-sm text-zinc-600 dark:text-zinc-400">
              Gerencie as chaves de API para este projeto. Você pode criar novas chaves,
              revogar chaves existentes e visualizar as chaves ativas.
            </p>
            <div class="flex items-center justify-between bg-secondary dark:bg-background-dark rounded-2xl p-4">
              ...
              <div class="flex items-center ml-auto">
                ...
                <Button variant="ghost" size="icon" @click="copy(project.apiKey!)" :disabled="copied">
                  <FadeInOutTransition>
                    <Icon :icon="Check" :size="16" class="text-success" v-if="copied" />
                    <Icon :icon="Copy" :size="16" class="text-ghost" v-else />
                  </FadeInOutTransition>
                </Button>
              </div>
            </div>
            <div class="p-3 bg-warning/10 rounded-xl flex items-center flex-wrap gap-3">
              ...
              <p class="text-sm text-warning">
                <strong>Atenção</strong>: Revele ou copie suas chaves de API apenas quando
                necessário. Nunca compartilhe suas chaves de API publicamente ou com
      ...
      <Card>
        ...
        <template #content>
          <div class="flex items-center justify-between gap-3">
            <p class="text-sm text-zinc-600 dark:text-zinc-400">
              A exclusão do projeto é permanente e não pode ser desfeita. Todos os dados
              serão perdidos.
            </p>
    ...
    <template #modal-content>
      <p>
        ...
        <span class="font-semibold text-text! dark:text-text-dark!">
          Não se preocupe </span>, essa ação pode ser desfeita a qualquer momento.
      </p>
    </template>
  </Modal>
